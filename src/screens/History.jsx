...
import { Button, Card, Eyebrow, Sparkline } from '../components.jsx'
import { FALLBACK_HISTORY } from '../data.jsx'
import { listJobs } from '../services/api.js'

// Adapt a backend job row {job_id, status, submitted_at, score} to the UI shape
// History expects. Until the backend stores names/duration/grade, we synthesize
// reasonable placeholders so the table still renders.
function adaptJob(j) {
  const score = typeof j.score === 'number' ? j.score : 0
  return {
    id: j.job_id,
    date: (j.submitted_at || '').slice(0, 10) || '—',
    name: { fr: `Session ${j.job_id}`, en: `Session ${j.job_id}` },
    duration: j.duration_s || 0,
    score,
    grade: j.grade || (score >= 85 ? 'A' : score >= 70 ? 'B' : score >= 55 ? 'C' : 'D'),
    kind: { fr: j.status || 'Session', en: j.status || 'Session' },
  }
}

export default function History({ onNav }) {
  const { t, lang } = useT()
  const [list, setList] = useState(FALLBACK_HISTORY)
  ...
  useEffect(() => {
    ...
    listJobs().then(jobs => {
      ...
      const adapted = jobs
        ...
        .map(adaptJob)
      if (adapted.length) setList(adapted)
    })
    return () => { cancelled = true }
  }, [])
  const trend = list.map(h => h.score).reverse()
  const avg = (list.reduce((s, h) => s + h.score, 0) / list.length).toFixed(1)
  ...
  return (
    <div className="fade-in" style={{ padding: "48px 32px 64px", maxWidth: 1280, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: 32, flexWrap: "wrap", gap: 16 }}>
        <div>
          <Eyebrow style={{ marginBottom: 8 }}>{list.length} {lang === "fr" ? "sessions" : "sessions"}</Eyebrow>
      ...
      <Card padding={24} style={{ marginBottom: 16 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 2fr", gap: 24, alignItems: "center" }}>
          ...
          <div>
            ...
            <div className="display num" style={{ fontSize: 24, fontWeight: 700 }}>{Math.max(...list.map(h => h.score))}</div>
          </div>
          <div>
