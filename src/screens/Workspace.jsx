...
import { Button, Card, Eyebrow } from '../components.jsx'
import { uploadVideo } from '../services/api.js'

export default function Workspace({ onNav, fileState, setFileState }) {
  const { t, lang } = useT()
  const [file, setFile] = useState(fileState?.file || null)
  // Real File object — kept separate from the display metadata so the actual
  // bytes survive across re-renders and can be sent to the backend.
  const [realFile, setRealFile] = useState(fileState?.realFile || null)
  ...
  const useDemo = () => { setRealFile(null); setFile(fakeFile) }

  const start = async () => {
    setFileState({ file, realFile, kind, audience, goal, script })
    setUploadError(null)
    // Demo mode (no real file): skip upload, run simulated processing.
    if (!realFile) {
      sessionStorage.removeItem('nood_job_id')
      sessionStorage.removeItem('nood_report')
      sessionStorage.removeItem('nood_video_url')
      onNav("processing")
      return
    }
    setUploading(true)
    const result = await uploadVideo(realFile)
    setUploading(false)
    if (!result.ok) {
      setUploadError(result.error || 'Upload failed')
      return
    }
    sessionStorage.setItem('nood_job_id', result.job_id)
    sessionStorage.removeItem('nood_report')
    // Object URL lets the report screen play the local file without re-fetching.
    try {
      sessionStorage.setItem('nood_video_url', URL.createObjectURL(realFile))
    } catch {}
    onNav("processing")
  }
          ...
          {!file ? (
            ...
            <div style={{ padding: 24, flex: 1 }}>
              <div style={{ display: "flex", gap: 16, padding: 16, border: "1px solid var(--rule)", borderRadius: 12, marginBottom: 16, alignItems: "center" }}>
                ...
                <button onClick={() => { setFile(null); setRealFile(null) }} style={{
              ...
              <Button kind="primary" size="lg" icon={uploading ? "progress_activity" : "play_arrow"} onClick={start} disabled={uploading} style={{ width: "100%" }}>
                {uploading ? (lang === "fr" ? "Téléversement…" : "Uploading…") : t("ws.upload.analyze")}
              </Button>
              {uploadError && (
                <div style={{ marginTop: 12, padding: "10px 12px", background: "rgba(192,57,43,0.08)", border: "1px solid rgba(192,57,43,0.25)", borderRadius: 8, color: "var(--bad)", fontSize: 12.5 }}>
                  {uploadError}
                </div>
              )}
            </div>
