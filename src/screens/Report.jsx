...
import { Button, Card, Eyebrow, MetricBar, ScoreRing, StackedBar } from '../components.jsx'
import { FALLBACK_REPORT } from '../data.jsx'

export default function Report({ onNav, report }) {
  const { t, lang } = useT()
  // Prefer the prop, then sessionStorage (set by Processing on completion),
  // else the demo fallback. Memoized so re-renders don't re-parse JSON.
  const r = useMemo(() => {
    if (report) return report
    try {
      const stored = sessionStorage.getItem('nood_report')
