...
import { Button, Card, Eyebrow, MiniWaveform } from '../components.jsx'
import { getJobStatus, getJobResult } from '../services/api.js'

const POLL_INTERVAL_MS = 3000
const MAX_WAIT_MS = 5 * 60 * 1000  // 5 min — gives long videos room without hanging forever

export default function Processing({ onNav }) {
  const { t, lang } = useT()
  const [elapsed, setElapsed] = useState(0)
  const [error, setError] = useState(null)
  const total = 60 // seconds — animation timeline (real status overrides progression once polled)
  ...
  useEffect(() => {
    const jobId = sessionStorage.getItem('nood_job_id')

    // Animation tick — purely visual; runs in both real-job and demo modes.
    const tick = setInterval(() => {
      setElapsed(e => Math.min(e + 0.25, total))
    }, 100)

    // Demo mode: no job to poll, just wait the simulated duration then nav.
    if (!jobId) {
      const demoTimeout = setTimeout(() => onNav("report"), total * 1000 + 500)
      return () => { clearInterval(tick); clearTimeout(demoTimeout) }
    }

    // Real-job mode: poll the backend.
    let cancelled = false
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (status.status === 'done') {
          ...
          if (cancelled) return
          sessionStorage.setItem('nood_report', JSON.stringify(report))
          onNav('report')
          return
        }
        if (status.status === 'failed') {
          setError(status.error || (lang === 'fr' ? "L'analyse a échoué." : 'Analysis failed.'))
          return
        }
      } catch (err) {
        // Transient network errors: keep polling. The hard timeout guards the rest.
        if (cancelled) return
      }
      if (Date.now() - startedAt > MAX_WAIT_MS) {
        setError(lang === 'fr' ? "Délai dépassé. Réessayez plus tard." : 'Timed out. Please try again later.')
        return
      }
  ...
  return (
    <div className="fade-in" style={{ padding: "48px 32px 64px", maxWidth: 1280, margin: "0 auto" }}>
      ...
      {error && (
        <Card padding={16} style={{ marginBottom: 16, borderColor: "rgba(192,57,43,0.4)", background: "rgba(192,57,43,0.05)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16 }}>
            <div style={{ color: "var(--bad)", fontSize: 13.5 }}>{error}</div>
            <Button kind="ghost" size="sm" onClick={() => onNav("workspace")}>{lang === "fr" ? "Réessayer" : "Try again"}</Button>
          </div>
        </Card>
      )}
